...
class InvoiceTable(QTableWidget):
    ...
    def set_records(self, records):
        ...
        # 自动调整列宽：设为 Interactive 让用户可拖拽列边框调整
        header = self.horizontalHeader()
        DEFAULT_WIDTHS = {
            "general": [40, 130, 160, 110, 220, 200, 110, 60, 90, 110, 100],
            "small": [40, 130, 160, 110, 220, 200, 110],
        }
        widths = DEFAULT_WIDTHS.get(self.taxpayer_type, DEFAULT_WIDTHS["small"])
        for c in range(min(data_cols, len(widths))):
            header.setSectionResizeMode(c, QHeaderView.Interactive)
            self.setColumnWidth(c, widths[c])
        # 剩余列（如果多于默认宽度定义）采用拉伸
        for c in range(len(widths), data_cols):
